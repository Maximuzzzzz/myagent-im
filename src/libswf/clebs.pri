...
#    isEmpty(3)|clebsCheckModules($$3):for(subdirs, 1) {
#        entries = $$files($$subdirs)
#        for(entry, entries) {
#            name = $$replace(entry, [/\\\\], _)
#            contains(SUBDIRS, $$name):next()
#            SUBDIRS *= $$name
...
defineTest(clebsCheck) { 
    contains(CLEBS, $$1)|contains(CLEBS_INSTALL, $$1)|contains(CLEBS_REQUIRED, $$1)|contains(CLEBS_SUGGESTED, $$1):return(true)
    return(false)
}
...
# General settings =============================================================
unix|win32-x-g++*:BASEDIR = $$system(pwd)
else:BASEDIR = $$system(cd)
...
# isEmpty(DATADIR):DATADIR = $$PREFIXDIR/share/swfdec-qt4
# isEmpty(DOCDIR):DOCDIR = $$PREFIXDIR/share/doc/swfdec-qt4
...
# isEmpty(PLUGINDIR):PLUGINDIR = $$PREFIXDIR/lib/swfdec-qt4
# }
# win32 {
...
for(ever) { 
    ...
    for(ever) { 
        ...
        !contains(libsincluded, $$dep) { 
            ...
            subdirs = clebs \
                $$files(clebs/*)
            for(subdir, subdirs):exists($${subdir}/$${dep}.pri) { 
